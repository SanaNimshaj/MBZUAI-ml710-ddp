...
class DGC(nn.Module):
    def __init__(self, model, rank, size,device_id, momentum, full_update_layers, percentages, iterations):
        ...
        for i, (name, layer) in enumerate(model.named_parameters()):
            if layer.requires_grad and len(layer.size()) == 4 and i not in self.full_update_layers:
                self.layers[name] = torch.zeros(layer.size()).cuda(self.rank)
    ...
    def avarage_gradient_dense(self):
        ...
        for i, (name, p) in enumerate(self.main_model.named_parameters()):
            tensor = p.grad.data.cuda(self.rank)
    ...
    def select_top_values_and_indices(self, tensor, name=None, momentum=None):
        ...
        small_values_tensor = small_values_tensor.put_(top_indices, torch.zeros(len(top_indices)).cuda(self.rank))

        self.layers[name] = self.layers[name].put_(top_indices, torch.zeros(len(top_indices)).cuda(self.rank))
    ...
    def accumulate_gradients(self):
        ...
        for i, (name, p) in enumerate(self.main_model.named_parameters()):
            ...
            if len(p.grad.data.cpu().shape) == 4:
                top_indices, top_values = self.select_top_values_and_indices(p.grad.data.cuda(self.rank),
                                                                                           name, self.momentum)
            ...
            if len(param.grad.data.cpu().shape) == 4:
                ...
                if self.rank == 0:
                    ...
                    for i in range(self.size):
                        g_list.append(torch.zeros(layer.shape).cuda(self.rank))
                    dist.gather(tensor=layer.cuda(self.rank), dst=0, gather_list=g_list)
                    ...
                    dist.gather(tensor=layer.cuda(self.rank), dst=0, gather_list=[])
